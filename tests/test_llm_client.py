...
from llm_client import get_client, OllamaClient, ClaudeClient, LLMClient
...
def test_ollama_client_config_from_env(monkeypatch):
    monkeypatch.setenv("OLLAMA_MODEL", "kimi-k2.6:cloud")
    monkeypatch.setenv("OLLAMA_HOST", "http://192.168.50.113:11434")
    client = OllamaClient()
    assert client.model == "kimi-k2.6:cloud"
    assert client.host == "http://192.168.50.113:11434"


def test_ollama_client_explicit_args_override_env(monkeypatch):
    monkeypatch.setenv("OLLAMA_MODEL", "from-env")
    client = OllamaClient(model="explicit", host="http://h:1")
